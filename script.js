...
function renderWorks(filter = "all") {
  ...
  const filtered = allWorks
    ...
    .sort((a, b) =>
      normalizeKana(a.reading).localeCompare(normalizeKana(b.reading), "ja")
    );
  ...
  list.innerHTML = filtered.map(w => {
    ...
    const caution = w.caution
      ? `<span class="work-caution">${w.caution}</span>`
      : "";
